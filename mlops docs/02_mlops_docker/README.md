...
``` 
chmod +x wait-for-it.sh prod/init_script.sh test/init_script.sh
```
- Fix sh file if broken due to Windows file system
```bash
...
```
### 2.2. Create all the containers
```
docker-compose up --build -d
```

- This may take approximately 10 mins.
...
```
- On mac change train with your username:everyone
```commandline
sudo chown -R erkansirin:everyone  prod/home/
sudo chown -R erkansirin:everyone  test/home/
```
...
### 5.7. Create runner Token

![alt text](create_gitea_runner_token_on_gitea_ui_1.png)

-----------


![alt text](create_gitea_runner_token_on_gitea_ui_2.png)
...
```
- Go check runner on gitea UI

![alt text](gitea_runner_registration_on_gitea_ui.png)
